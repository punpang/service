    ...
    public static function socialPaymentOrder($order, $amount, $is_pickup = false)
    {
        if ($is_pickup != true) {
            ...
            if ($order->orderDeliveryService) {
                $msgFacebook = "ขอบคุณที่ชำระเงินค่ะ
--------------------
เราจะแจ้งคุณอีกครั้ง เมื่อเริ่มจัดส่งสินค้าของคุณ
...
--------------------
วัน-เวลานัดรับสินค้า
" . $order->date_get_th . " " . $order->time_get_format . " น.
--------------------
- เวลานัดรับข้างต้น เป็นเพียงเวลาที่สินค้าจะจัดเตรียมเสร็จ
