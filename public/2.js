...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    status_payment_deadline: function status_payment_deadline() {
      var now = new Date();
      var nowParse = Date.parse(now.toLocaleString());
      var deadlineParse = Date.parse(this.order.payment_deadline);
      return deadlineParse > nowParse;
    },
    ntpfcsLength: function ntpfcsLength(v, d) {
        ...
        [
          _vm.order.sumAll.sumDeposited === 0
            ? _c("strong", [
                _vm._v(
                  "โปรดชำระเงินก่อน " +
                    _vm._s(_vm.order.payment_deadline) +
                    " น."
                ),
              ])
            : _c("strong", [
