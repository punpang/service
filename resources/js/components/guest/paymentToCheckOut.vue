    ...
    <v-dialog persistent scrollable width="500" v-model="dialog">
      ...
      <v-card>
        ...
        <v-card-text>
            ...
            <v-card-text>
              <v-simple-table>
                <tr>
                  ...
                  <td class="text-right">
                    <strong> {{ propDeposit | formatNumber }} บาท </strong>
                  </td>
                </tr>

                <tr>
                  <td>
                    <strong> {{ paymentChannel.remark }}</strong>
                  </td>
                  <td class="text-right">
                    <strong> {{ fee_value | formatNumber }} บาท </strong>
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td class="text-right">
                    <strong> {{ amount | formatNumber }} บาท </strong>
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td class="text-right">
                    <strong> {{ paymentChannel.text }}</strong>
                  </td>
                </tr>
              </v-simple-table>
            </v-card-text>
          ...
          <v-card outlined>
            <v-card-text v-if="propChannelID === 1" class="align-center">
              ...
              <v-flex class="text-center my-2">
                ...
                <v-img
                  ...
                  :src="ksherPay.imgdat"
                ></v-img>
                <strong>โปรดทำการชำระเงินเพียงครั้งเดียว</strong>
                <!-- :src="qrcodeTest" -->
              </v-flex>
              <p class="mb-1">1. แคปหน้าจอหน้านี้ โดยให้เห็น QR CODE เต็มรูป</p>
              <p class="mb-1">
              ...
              <p class="mb-1">4. เลือกรูปภาพที่คุณแคปไว้และทำการชำระเงิน</p>
              <p class="mb-1">
            ...
            <v-card-text v-else-if="propChannelID >= 2 && propChannelID <= 4">
              <v-alert type="warning" text
                >โปรดตรวจสอบยอดชำระก่อน <strong>"ชำระเงิน"</strong></v-alert
              >
              <v-alert type="info" text>ไม่ต้องแจ้งชำระเงิน</v-alert>

              <strong>ทางร้านใช้ระบบชำระเงินของ Ksher Pay</strong>
              <v-spacer></v-spacer>

              <v-btn class="success" x-large block @click="ksherPayOrderCreate">
                <v-icon left>payments</v-icon>
                ชำระเงิน
              </v-btn>
            </v-card-text>
...
export default {
  ...
  mounted() {
    window.Echo.channel("KsherPayEventOn").listen(".KsherPayEventAs", (e) => {
      const data = e.data;
      if (
        data.result === "success" &&
