  ...
  <div>
    ...
    >
      <strong v-if="order.sumAll.sumDeposited === 0"
        >โปรดชำระเงินก่อน {{ order.payment_deadline }} น.</strong
      >
      <strong v-else>ขอบคุณที่ชำระเงินเพื่อยืนยันการสั่งซื้อค่ะ</strong>
      <p
        class="mb-0"
        v-show="!status_payment_deadline() && order.sumAll.sumDeposited === 0"
      >
      ...
      <formPayment
        class="mt-1"
        v-if="
          (status_payment_deadline() && order.sumAll.sumDeposited === 0) ||
          (order.sumAll.sumDeposited > 0 && order.sumAll.sumBalance > 0)
        "
      ></formPayment>
    </v-alert>
...
export default {
  ...
  methods: {
    async start() {
      ...
      const datas = await this.$store.dispatch("orderIndex/getOrderByUUID", {
